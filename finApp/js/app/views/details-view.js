...
define([
    ...
    function render(company) {
        ...
        $('#app').empty();
        var $details = drawDetails(company);
        // $('#app').html(listItem(createItems(company), ticker));
    ...
    function drawDetails(company) {
        ...
        var $details = $('<div>').addClass('ui raised segment').css('text-align','justify');
        ...
        $details.append('<p>' + company.hqState + ', ' + company.hqCountry + '<br>');
        $details.append('<h5> CEO: </h5>' );
        $details.append('<p>' + company.ceo);
        $details.append('<h5> Industry: </h5>' );
        $details.append('<p>' + company.sector + ', ' + company.industryGroup + ', ' + company.industryCategory);
